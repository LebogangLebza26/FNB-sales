...
SELECT
    ...
    Sales,
    `Quantity_Sold`,
    ROUND(Sales / `Quantity_Sold`, 4) AS price_per_unit
FROM fnbdataset.sales.fnb_sales_2021
ORDER BY date;
...
SELECT
    ROUND(AVG(`Cost_Of_Sales`), 4)                   AS avg_price_simple,
    ROUND(SUM(Sales) * 1.0 / SUM(`Quantity_Sold`), 4) AS avg_price_volume_weighted
FROM fnbdataset.sales.fnb_sales_2021;
...
SELECT
    Date,
    ROUND((Sales - `Cost_Of_Sales`) / sales * 100, 4) AS gross_profit_pct
FROM fnbdataset.sales.fnb_sales_2021;
...
SELECT
   ...
   Sales,
    `Cost_Of_Sales`,
    ROUND((Sales - `Cost_Of_Sales`) / Sales * 100, 4) AS gross_profit_pct_per_unit
FROM fnbdataset.sales.fnb_sales_2021;

day_nameDatefnbdataset.sales.fnb_sales_2021
----------------------------------------------------------------
--Promo period
SELECT
    Date,
    ROUND(AVG(`Cost_Of_Sales`) OVER (
        ORDER BY Date
...
SELECT Date,`Cost_Of_Sales`, `Quantity_Sold`,
    CASE
...
WITH v_period_summary AS (
    SELECT
        ...
        ROUND(AVG(`Cost_Of_Sales`), 4) AS avg_price,
        ROUND(AVG(`Quantity_Sold`), 1) AS avg_qty
...
SELECT
    ...
    ROUND(AVG(t.Sales - t.`Cost_Of_Sales`), 2) AS avg_daily_gross_profit_rand,
    ROUND(AVG((t.Sales - t.`Cost_Of_Sales`) / t.Sales * 100), 2) AS avg_gross_profit_pct
...
SELECT
    ...
    ROUND(AVG((Sales - `Cost_Of_Sales`) / Sales * 100), 2) AS avg_gross_profit_pct
FROM fnbdataset.sales.fnb_sales_2021
...
-- Day-of-week pattern
SELECT
    DAYOFWEEK(day_name) AS day_of_week,
